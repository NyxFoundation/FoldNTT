...
non-negative, so no signed hardware is needed):
...
conditional subtraction. `modular_mul_kred.v` implements it, pipelined to
the same latency 4 as the original.

### 3. Folding the factor 9 away

The unit returns `9ab mod q`, not `ab mod q`. The factor disappears
architecturally, with no correction hardware on the hot path:
...
scaled. The companion invention in [`../psi-fold-rom/`](../psi-fold-rom/)
then halves the ROM itself.
...
operating mode the flattened v2 tree is feed-forward with an input→output
register path of exactly 6, independently confirming the latency the
