...
`{=latex}` raw block via `gfm+raw_attribute` and emitted as a
both-column-spanning `figure*` with `\resizebox`, so it renders in both
the single-column and two-column builds. The datapath, the shaded changed
blocks, and both adder/subtractor inputs all render cleanly in two columns.
