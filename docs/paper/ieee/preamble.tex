...
% 3.5in column.
% Fonts by FILE name (not family name): arXiv's XeLaTeX has no fontconfig
% database, so name lookups fail there; file lookups work in any TeX Live.
% Full file names (with extension) so arXiv's source scanner recognises them;
% the arXiv package also ships these files next to main.tex.
