...
run as-is on NixOS (no `/lib64/ld-linux…`, no FHS). Three working
approaches follow, easiest first. AMD/Xilinx Vivado ML Standard is free and
includes the Artix-7 devices CFNTT targets, so no paid license is needed
for this.
...
point `settings64.sh` there. The `ncurses5`/`ncurses6` mismatch is the usual
...
is leaner. Verify the flake is current before relying on it, since Vivado
...
flow. It needs no license, runs in the existing Nix shell, and tells us
...
1. `synth_xilinx` per module now (the pre-check above) → first LUT/DSP/BRAM
   table; confirms fold7/K-RED timing direction. No license needed; runs
   today.
