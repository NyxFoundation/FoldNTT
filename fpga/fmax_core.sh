...
# reference top_poly_mul vs proposed top_poly_mul_v2.  The released fsm.v is
# empty, so the core is linked against the reconstructed fullcore/fsm_recon.v
# (the controller verification/fullcore/run_sim.py validates end-to-end).
# keep on datapath leaf modules prevents DCE.  Best of 3 placer seeds; each
# seed's nextpnr log is archived under fpga/reports/.  Requires NP, CHIPDB
# (see fmax.sh / README).
set -euo pipefail
